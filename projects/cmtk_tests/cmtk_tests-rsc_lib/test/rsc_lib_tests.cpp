#include <cmtk_tests/assets/assets.hpp>
#include <cmtk_tests/assets/icon.hpp>
#include <cmtk_tests/assets/text/tale.hpp>
#include <cmtk_tests/assets/text/another_tale.hpp>
#include <cmtk_tests/assets/text/third_tale.hpp>
#include <cmtk_tests/assets/text/fourth_tale.hpp>
#include <cmtk_tests/assets/text/short_message/message.hpp>
#include <gtest/gtest.h>

TEST(rsc_lib_tests, resource_icon__path_and_size__ok)
{
    EXPECT_EQ(assets::icon_path, "RSC_LIB:/icon.png");
    EXPECT_EQ(cmtk_tests::assets::icon().size(), 5465);
}

TEST(rsc_lib_tests, resource_tale__path_and_size__ok)
{
    EXPECT_EQ(cmtk_tests::assets::text::tale_path, "RSC_LIB:/text/tale.txt");
    EXPECT_EQ(cmtk_tests::assets::text::tale().size(), 19);
}

TEST(rsc_lib_tests, resource_message__path_and_size__ok)
{
    EXPECT_EQ(cmtk_tests::assets::text::short_message::message_path, "RSC_LIB:/text/short_message/message.txt");
    EXPECT_EQ(cmtk_tests::assets::text::short_message::message().size(), 6);
}

TEST(rsc_lib_tests, find_serialized_resource__tale__found)
{
    std::string_view rsc_path = "RSC_LIB:/text/tale.txt";
    std::optional rsc_bytes_o = cmtk_tests::assets::find_serialized_resource(rsc_path);
    EXPECT_TRUE(static_cast<bool>(rsc_bytes_o));
    EXPECT_EQ(rsc_bytes_o->data(), cmtk_tests::assets::text::tale().data());
    EXPECT_EQ(rsc_bytes_o->size(), cmtk_tests::assets::text::tale().size());
...
TEST(rsc_lib_tests, find_serialized_resource__another_tale__found)
{
    ...
    std::optional rsc_bytes_o = cmtk_tests::assets::find_serialized_resource(rsc_path);
    EXPECT_TRUE(static_cast<bool>(rsc_bytes_o));
    EXPECT_EQ(rsc_bytes_o->data(), cmtk_tests::assets::text::another_tale().data());
...
TEST(rsc_lib_tests, find_serialized_resource__third_tale__found)
{
    ...
    std::optional rsc_bytes_o = cmtk_tests::assets::find_serialized_resource(rsc_path);
    EXPECT_TRUE(static_cast<bool>(rsc_bytes_o));
    EXPECT_EQ(rsc_bytes_o->data(), cmtk_tests::assets::text::third_tale().data());
...
TEST(rsc_lib_tests, find_serialized_resource__fourth_tale__found)
{
    ...
    std::optional rsc_bytes_o = cmtk_tests::assets::find_serialized_resource(rsc_path);
    EXPECT_TRUE(static_cast<bool>(rsc_bytes_o));
    EXPECT_EQ(rsc_bytes_o->data(), cmtk_tests::assets::text::fourth_tale().data());
    EXPECT_EQ(rsc_bytes_o->size(), 29);
    std::string_view content(reinterpret_cast<const char*>(rsc_bytes_o->data()), rsc_bytes_o->size());
...
TEST(rsc_lib_tests, find_serialized_resource__message__found)
{
    ...
    std::optional rsc_bytes_o = cmtk_tests::assets::find_serialized_resource(rsc_path);
    EXPECT_TRUE(static_cast<bool>(rsc_bytes_o));
    EXPECT_EQ(rsc_bytes_o->data(), cmtk_tests::assets::text::short_message::message().data());
